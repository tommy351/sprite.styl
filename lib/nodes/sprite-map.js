...
SpriteMap.prototype.findMap = function(name){
  var list = Object.keys(this.map),
    length = list.length,
    i = 0;
  ...
  for (i = 0; i < length; i++){
    ...
    if (item.substring(0, item.length - pathFn.extname(item).length) === name){
      return this.map[list[i]];
    }
  }
  ...
  // Only base name
  for (i = 0; i < length; i++){
    if (pathFn.basename(list[i]) === name){
      return this.map[list[i]];
    }
  }
  ...
  for (i = 0; i < length; i++){
    ...
    if (pathFn.basename(item, pathFn.extname(item)) === name){
      return this.map[list[i]];
    }
  }
};
...
SpriteMap.prototype.position = function(name, offsetX, offsetY){
  ...
  var map = this.findMap(name);

  return {
    x: map ? -map.x - offsetX : 0,
    y: map ? -map.y - offsetY : 0
  };
};